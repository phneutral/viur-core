...
class Query(object):
	...
	def __init__(self, kind: str, srcSkelClass: Union[None, Any] = None, *args, **kwargs):
		...
		# Some (Multi-)Queries need a different limit of results per subQuery than actually returned
	...
	def run(self, limit=-1, **kwargs):
		...
		if self.filters is None:
			return None
		origLimit = limit if limit != -1 else self.limit
		qryLimit = origLimit
	...
	def fetch(self, limit=-1, **kwargs):
		...
		if self.srcSkel is None:
			raise NotImplementedError("This query has not been created using skel.all()")
		limit = limit if limit != -1 else self.limit
